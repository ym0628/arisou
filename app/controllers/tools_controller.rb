class ToolsController < ApplicationController
  ...
  def show
    ...
    if unit_number_array.size != unit_number_array.uniq.size
      ...
      flash.now[:warning] = "台番号 #{duplication_number} が#{t('defaults.message.duplication')}"
    end
    # 配列に作り直す
    tool_units_array = @tool.tool_units.pluck(:number, :medal)
    # 差枚数がプラスの要素だけを取り出した配列を作り、さらに差枚プラスの台番だけを残した配列に直す
    win_unit = tool_units_array.select { |_key, value| value.positive? }.pluck(0)
    # 各末尾(0~9)の勝っている台数をカウント
    @win_count = win_unit.map { |n| n.digits[0] }.group_by { |a| a }.transform_values(&:count)
    # 末尾ゾロ目かつ差枚プラス台の台数をカウント
    @win_same_number_count = win_unit.select { |n| n > 10 }.map { |n| n.digits[0..1] }.count { |a, b| (a - b).zero? }
    # 各末尾ごとの勝率
    @win_rate0 = ((@win_count[0].to_i / @each_denominator) * 100).round(1)
    @win_rate1 = ((@win_count[1].to_i / @each_denominator) * 100).round(1)
    @win_rate2 = ((@win_count[2].to_i / @each_denominator) * 100).round(1)
    @win_rate3 = ((@win_count[3].to_i / @each_denominator) * 100).round(1)
    @win_rate4 = ((@win_count[4].to_i / @each_denominator) * 100).round(1)
    @win_rate5 = ((@win_count[5].to_i / @each_denominator) * 100).round(1)
    @win_rate6 = ((@win_count[6].to_i / @each_denominator) * 100).round(1)
    @win_rate7 = ((@win_count[7].to_i / @each_denominator) * 100).round(1)
    @win_rate8 = ((@win_count[8].to_i / @each_denominator) * 100).round(1)
    @win_rate9 = ((@win_count[9].to_i / @each_denominator) * 100).round(1)
    @win_rate_same_number = ((@win_same_number_count.to_i / @each_denominator) * 100).round(1)
    ...
    same_number_array = tool_units_array.select { |n, _m| n >= 10 }.select { |n, _m| n.digits[0] == n.digits[1] }
    # 末尾ゾロ目の総差枚数・平均差枚数を計算
